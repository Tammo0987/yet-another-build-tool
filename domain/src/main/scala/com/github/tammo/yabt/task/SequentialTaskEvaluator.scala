...
object SequentialTaskEvaluator extends TaskEvaluator:
  ...
  override def evaluateTask[T](task: Task[T]): Result[T] =
    val result = evaluateComputation(task.computation)

    // FIXME: Logging as decorator
    val prefix = resultToPrefix(result)
    println(s"[$prefix] Evaluated task ${task.info.name}")

    result

  private def evaluateComputation[T](computation: Computation[T]): Result[T] =
    computation match
  ...
  private def evaluateJoin[T, F, S](join: Join[T, F, S]): Result[T] = {
  ...
  private def evaluateDependsOn[F, T](dependsOn: DependsOn[F, T]): Result[T] = {
    evaluateTask(dependsOn.taskDependency)
    evaluateComputation(dependsOn.task.computation)
  }
